...
def testSingleCase(student_func, case_args, case_expected) -> tuple:
    ...
    try:
        ...
        if not compare:
            return (TestResult.INCORRECT, wrongAnsMsgGenerator(case_expected=case_expected, stud_ans=stud_ans))
...
def testQ2(student_func, err_code):
    ...
    return simpleTester(student_func, deepcopy(cases), expected, err_code)
