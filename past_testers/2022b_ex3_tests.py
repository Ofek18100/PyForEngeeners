...
def testQ1(student_func, err_code):
    ...
    expected = [6.0, 1.0, 1.0, 3136.0, 8.0, 1.0]
...
def testQ4(student_func, err_code):
    ...
    for case_num, (case_args, case_expected) in enumerate(zip(cases, expected), 1):
        try:
            ...
            if (case_num in [1,2, 3] and case_args != case_expected):
            ...
            elif (case_num in [4,5] and stud_ans!=case_expected):
                msg = wrongAnsMsgGenerator(case_expected, stud_ans)
                err_dict.update(proccessTestRes(err_code, case_num, TestResult.INCORRECT, msg))
...
def testQ5(student_func, err_code):
    err_dict = {}
    multipliers = [-2, 2, 9, 5, 1]
    cases = [(deepcopy(mat), multiplier) for mat, multiplier in zip(mat_list, multipliers)]
    expected = [np.array(mat)*multiplier for mat, multiplier in zip(mat_list, multipliers)]
